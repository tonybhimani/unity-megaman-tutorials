...
public class BlasterController : MonoBehaviour
{
    ...
    // delay time before first attack
    ...
    void Update()
    {
        ...
        // do Blaster ai logic if it's enabled
        if (enableAI)
        {
            // get player position
            if (player != null) playerPosition = player.transform.position;

            // if there is a start delay then countdown till it's over
            if (startDelay > 0)
            {
    ...
    public void SetOrientation(BlasterOrientation orientation)
    {
        // set Blaster orientation
