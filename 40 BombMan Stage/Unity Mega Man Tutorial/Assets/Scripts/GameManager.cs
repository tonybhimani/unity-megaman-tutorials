...
public class GameManager : MonoBehaviour
{
    ...
    public void ShowWeaponsMenu()
    {
        ...
        // two actions occur when player weapons are in the scene and showing the weapons menu
        //   1. if the game is paused then unpause it and return to play
        //   2. if not paused then ignore the request until the weapons are gone
        // otherwise when there are no weapons in the scene then switch between options normally

        // check for any player weapons active in the scene
        if (ArePlayerWeaponsInScene())
        {
            // is the game is paused? then unpause it
            if (isGamePaused)
            {
                PauseGame(true);
            }
            // ignore the request when weapons are in the scene
            return;
        }
    ...
    private void GetWorldViewCoordinates()
    {
        ...
        Vector3 wv0 = Camera.main.ViewportToWorldPoint(new Vector3(-0.05f, -0.05f, 0));
        Vector3 wv1 = Camera.main.ViewportToWorldPoint(new Vector3(1.05f, 1.05f, 0));
